...
PROPAGATION_SECONDS = int(os.getenv("DNS_PROPAGATION_SECONDS", "25"))
...
def cf_create_txt(token: str, base_domain: str, value: str):
    zone_id = cf_find_zone_id(token, base_domain)
    name = f"_acme-challenge.{base_domain}"
    payload = {"type": "TXT", "name": name, "content": value, "ttl": 120}
    cf_api_request(token, "POST", f"/zones/{zone_id}/dns_records", payload=payload)
...
def main():
    ...
    if not requested_domain or not validation:
        print("Faltan variables CERTBOT_DOMAIN o CERTBOT_VALIDATION", file=sys.stderr)
        return 1

    spec = get_domain_spec(args.domain_id, requested_domain)
    if not spec:
        print(f"No hay mapeo provider/credencial para dominio {requested_domain}", file=sys.stderr)
        return 1
    ...
    if provider == "cloudflare":
        ...
        if not token:
            print("Falta API token Cloudflare", file=sys.stderr)
            return 1
        cf_create_txt(token, base_domain, validation)
    elif provider == "aws":
        ...
        if not access_key or not secret_key:
            print("Faltan credenciales AWS para Route53", file=sys.stderr)
            return 1
        route53_change_txt(access_key, secret_key, region, base_domain, validation, "UPSERT")
    else:
        print(f"Proveedor no soportado: {provider}", file=sys.stderr)
        return 1
